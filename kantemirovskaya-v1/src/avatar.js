import {garmentGeometry,finishOutfit,fitGarmentDetails} from './wardrobe.js';
import { T, mat, mesh, box, sphere, cyl, canvasTexture, isSharedGeometry } from './graphics.js';
...
const palettes=[['#414e48','#303e47','#c9c5b4'],['#a26342','#283c49','#e2d8bb'],['#cbd5cf','#344751','#dedac5'],['#293d53','#293d53','#e6dfce'],['#cec1a0','#283b44','#f2e8cc']];
function materialSet(hero,stage){const key=hero+':'+stage;if(looks.has(key))return looks.get(key);const h=HEROES[hero],p=palettes[stage];
 ...
 face.onBeforeCompile=s=>{s.vertexShader=s.vertexShader.replace('#include <common>','#include <common>\nattribute float faceWeight; varying float vFaceWeight;').replace('#include <begin_vertex>','#include <begin_vertex>\nvFaceWeight=faceWeight;');s.fragmentShader=s.fragmentShader.replace('#include <common>','#include <common>\nvarying float vFaceWeight;').replace('#include <map_fragment>','#ifdef USE_MAP\nvec4 faceSample=texture2D(map,vMapUv);diffuseColor*=vec4(mix(vec3(.405,.275,.205),faceSample.rgb,clamp(vFaceWeight,0.,1.)),1.);\n#endif');};face.customProgramCacheKey=()=> 'face-projection-1';
 const hair=mat(h.hair,.93);
 if(!hair.bumpMap){const grain=canvasTexture(256,256,(c,w,h)=>{c.fillStyle='#777';c.fillRect(0,0,w,h);for(let i=0;i<1800;i++){const x=(i*73)%256,y=(i*131)%256;c.strokeStyle=i%2?'#aaaaaa':'#505050';c.lineWidth=.6;c.beginPath();c.moveTo(x,y);c.quadraticCurveTo(x+2,y-4,x+1,y-9);c.stroke();}});grain.colorSpace=T.NoColorSpace;hair.bumpMap=grain;hair.bumpScale=.0012;}
 const set={head:face,skin,shirt:fabric(p[0],stage>=3?'wool':'cotton'),pants:fabric(p[1],'wool'),shoes:fabric(stage>=3?'#26211d':'#55564d','leather'),hair};looks.set(key,set);return set;
}
function panel(parent,pts,material){const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(pts.flat(),3));const ix=[];for(let i=1;i<pts.length-1;i++)ix.push(0,i,i+1);g.setIndex(ix);g.computeVertexNormals();const n=g.attributes.normal;let facing=0;for(let i=0;i<n.count;i++)facing+=n.getZ(i);if(facing<0){for(let i=0;i<ix.length;i+=3)[ix[i],ix[i+1]]=[ix[i+1],ix[i]];g.setIndex(ix);g.computeVertexNormals();}return mesh(parent,g,material);}
...
export class Avatar{
 ...
 setLook(hero,stage){this.hero=hero;this.stage=stage;if(this.skeleton)this.skeleton.dispose();this.body.traverse(o=>{if(o.isMesh&&!o.userData.avatarBase&&!isSharedGeometry(o.geometry))o.geometry.dispose();});this.body.clear();this.body.scale.set([1.10,.96,1.035,1.02][hero],[1.02,.98,1,1.015][hero],1.025);
  ...
  const materials=materialSet(hero,stage);for(const[name,g]of Object.entries(geometries)){const m=new T.SkinnedMesh(garmentGeometry(g,name,stage),materials[name]);m.userData.avatarBase=true;m.name=name;m.castShadow=m.receiveShadow=true;m.frustumCulled=false;this.body.add(m);m.bind(this.skeleton);}
  ...
  this.detail.traverse(o=>{if(o.isMesh){const p=o.geometry.attributes.position;if(p&&o.geometry.type==='BufferGeometry'){for(let i=0;i<p.count;i++)if(p.getZ(i)>.04)p.setZ(i,p.getZ(i)+.019);p.needsUpdate=true;}}});
  finishOutfit(this,stage);fitGarmentDetails(this,stage);
 }
 update(t,moving,yaw){const w=Math.sin(t*9.5),b=this.bones;this.body.position.y=Math.abs(Math.sin(t*9.5))*moving*.015;b.hips.rotation.z=w*moving*.025;b.chest.rotation.y=w*moving*.045;b.chest.rotation.x=moving*.035+Math.sin(t*1.7)*.004;b.head.rotation.y=Math.sin(t*.65)*.022;
  for(const[side,s]of[['l',1],['r',-1]]){b[side+'Thigh'].rotation.x=w*s*moving*.52;b[side+'Shin'].rotation.x=Math.max(0,-w*s)*moving*.72;b[side+'Foot'].rotation.x=-Math.max(0,w*s)*moving*.12;b[side+'Arm'].rotation.x=-w*s*moving*.38;b[side+'Arm'].rotation.z=-s*(.035+Math.sin(t*1.35)*.007)*(1-moving);b[side+'ForeArm'].rotation.x=-.08-Math.max(0,w*s)*moving*.20;}
