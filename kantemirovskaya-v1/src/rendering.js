...
export async function lighting(renderer,scene){
 ...
 scene.background=hdr;scene.backgroundIntensity=.82;scene.backgroundBlurriness=.025;
 scene.backgroundRotation.y=.65;scene.environmentRotation.y=.65;
 scene.fog=new T.Fog('#c6cfd0',95,240);pmrem.dispose();
 scene.add(new T.HemisphereLight('#d6e3f2','#716953',.60));
 const sun=new T.DirectionalLight('#fff0d4',3.2);sun.castShadow=true;
 sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-34,right:34,top:34,bottom:-34,near:1,far:170});
 sun.shadow.bias=-.000075;sun.shadow.normalBias=.018;sun.shadow.radius=2;
 sun.position.set(-40,51,39);sun.target.position.set(-22,0,24);scene.add(sun,sun.target);
 renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.98;
 return sun;
}
...
export class Picture {
 ...
 setup(){
  ...
  ao.kernelRadius=8;ao.minDistance=.0002;ao.maxDistance=.022;
