...
function foliageMaterial(){if(leafMat)return leafMat;const tex=canvasTexture(512,512,c=>{
 ...
 for(let n=0;n<70;n++){const y=50+r()*360,x=250+(r()-.5)*Math.sqrt(Math.sin(y/512*Math.PI))*390,a=(r()-.5)*2.7,sz=13+r()*18;c.save();c.translate(x,y);c.rotate(a);const light=75+r()*55;c.fillStyle=`rgb(${light*.79},${light},${light*.45})`;c.beginPath();c.moveTo(0,-sz);c.bezierCurveTo(sz*.75,-sz*.35,sz*.55,sz*.40,0,sz);c.bezierCurveTo(-sz*.55,sz*.4,-sz*.75,-sz*.35,0,-sz);c.fill();c.strokeStyle='#b6bb6980';c.lineWidth=.65;c.beginPath();c.moveTo(0,-sz*.8);c.lineTo(0,sz*.82);c.stroke();c.restore();}
...
export function createTrees(parent,buildings){
 ...
 const count=spots.length*34,leaves=new T.InstancedMesh(new T.PlaneGeometry(1,1,1,1),foliageMaterial(),count),o=new T.Object3D();let n=0;
 spots.forEach(([x,z,h])=>{
  ...
  for(let j=0;j<34;j++){const a=r()*Math.PI*2,dy=r()*2-1,rr=Math.sqrt(Math.max(.1,1-dy*dy))*(1.5+r()*1.35);o.position.set(x+Math.sin(a)*rr,h*.73+dy*h*.30,z+Math.cos(a)*rr);o.rotation.set((r()-.5)*2.4,r()*6.28,(r()-.5)*1.3);const sc=1.7+r()*.9;o.scale.set(sc,sc,1);o.updateMatrix();leaves.setMatrixAt(n,o.matrix);leaves.setColorAt(n,new T.Color().setHSL(.18+r()*.06,.11+r()*.14,.57+r()*.18));n++;}
 });leaves.castShadow=leaves.receiveShadow=true;leaves.userData.noAO=true;parent.add(leaves);return leaves;
}
export function paverMaterial(){const tex=canvasTexture(512,512,c=>{const r=rng(77);c.fillStyle='#706f64';c.fillRect(0,0,512,512);for(let y=0;y<512;y+=64)for(let x=-64;x<512;x+=128){const xx=x+(y%128?64:0),v=138+r()*21;c.fillStyle=`rgb(${v+7},${v+6},${v})`;c.fillRect(xx+2,y+2,123,59);c.fillStyle='#b2b1a580';c.fillRect(xx+3,y+3,122,2);for(let k=0;k<180;k++){c.fillStyle=k%2?'#45493c18':'#ffffec28';c.fillRect(xx+r()*124,y+r()*61,1,1);}}});tex.wrapS=tex.wrapT=T.RepeatWrapping;return new T.MeshStandardMaterial({map:tex,bumpMap:tex,bumpScale:.015,roughness:.96});}
export function addGroundDetails(parent,buildings,ways){
 const r=rng(341),blade=new T.BufferGeometry(),pos=[],col=[];for(let j=0;j<5;j++){const a=j*2.4,x=Math.cos(a)*.065,z=Math.sin(a)*.065,h=.09+j*.033,w=.017;pos.push(x-w,0,z,x+w,0,z,x+.045,h,z+.02);for(let k=0;k<3;k++)col.push(k===2?.4:.19,k===2?.45:.27,k===2?.19:.11);}blade.setAttribute('position',new T.Float32BufferAttribute(pos,3));blade.setAttribute('color',new T.Float32BufferAttribute(col,3));blade.computeVertexNormals();
